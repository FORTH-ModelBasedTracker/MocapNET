     ...
     if ( (bbox!=0) && (bbox->populated) )
      {
       ...
       //----------------------------------------------------------------------------------------------------------------------------------------------------
       if (inputWidth2DJointDetector >bodyWidth )    { bodyWidth = inputWidth2DJointDetector;   }
       if (inputHeight2DJointDetector> bodyHeight )  { bodyHeight = inputHeight2DJointDetector; }
      ...
      //----------------------------------------------------------------
       ...
       if (bbox->minimumX < 0 ) { limitLX=-1*bbox->minimumX;   bbox->minimumX = 0;  }
       if (bbox->minimumY < 0 ) { limitLY=-1*bbox->minimumY;   bbox->minimumY = 0;  }
       if (bbox->maximumX > fullFrameWidth )   {  limitRX=bbox->maximumX-fullFrameWidth;  bbox->maximumX = fullFrameWidth;   }
       if (bbox->maximumY > fullFrameHeight )  {  limitRY=bbox->maximumY-fullFrameHeight; bbox->maximumY = fullFrameHeight;  }
       ...
       if (limitLX>0)   {   bbox->maximumX += limitLX; }  
       if (limitLY>0)   {   bbox->maximumY += limitLY; }  

      //We are maybe constrained right/down we subtract what we lost left/up ..!
       if (limitRX>0)   {   bbox->minimumX -= limitRX; }  
       if (limitRY>0)   {   bbox->minimumY -= limitRY; }  
       ...
       if (bbox->minimumX < 0 ) { limitLX=1;   bbox->minimumX = 0;  }
       if (bbox->minimumY < 0 ) { limitLY=1;   bbox->minimumY = 0;  }
       if (bbox->maximumX > fullFrameWidth )   {  limitRX=1;  bbox->maximumX = fullFrameWidth;   }
       if (bbox->maximumY > fullFrameHeight )  {  limitRY=1;  bbox->maximumY = fullFrameHeight;  }
