...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (loadJointEstimator2D(
        ...
        {
            if ( loadMocapNET2(&mnet,"Webcam Demo") )
                {
                    //------------------------------
                    ...
                    if (  (options.save3DVisualization) || (options.save2DVisualization) )
                        {
                            ...
                            if (highResEncoding) 
                            {
                              snprintf(formatString,256,"ffmpeg -framerate %f -i vis%%05d.jpg  -s 1200x720  -y -r %f -pix_fmt yuv420p -threads 8 livelastRun3DHiRes.mp4 %s",options.inputFramerate,options.inputFramerate,eraseIntermediateImages);
                            } else
                            {
                              snprintf(formatString,256,"ffmpeg -framerate %f -i vis%%05d.jpg -y -r %f -threads 8 -crf 9 -pix_fmt yuv420p lastRun3D.webm %s",options.inputFramerate,options.inputFramerate,eraseIntermediateImages);  
                            }
