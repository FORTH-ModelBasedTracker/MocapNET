...
class FaceBVHAnimationPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.label(text="Neck : ")
        row = layout.row()
        row.prop(scene, 'neckZ', slider=True) 
        row = layout.row()
        row.prop(scene, 'neckX', slider=True)
        row = layout.row()
        row.prop(scene, 'neckY', slider=True) 
...
class FaceBVHAnimation(bpy.types.Operator):
    ...
    def retrieveFaceControlsI(self,context):
      ...
      #--------------------------------------------------------
      r["neck_Zrotation"]           = randomize_property("Neck Z")
      r["neck_Xrotation"]           = randomize_property("Neck X")
      r["neck_Yrotation"]           = randomize_property("Neck Y")
      #--------------------------------------------------------
    ...
    def retrieveFaceControlsFromCSV(self,context,csvData,fID):
      ...
      #--------------------------------------------------------
      r["neck_Zrotation"]           = resolveCSVRowColumn(csvData,"neck_Zrotation",fID)
      r["neck_Xrotation"]           = resolveCSVRowColumn(csvData,"neck_Xrotation",fID)
      r["neck_Yrotation"]           = resolveCSVRowColumn(csvData,"neck_Yrotation",fID)
      #--------------------------------------------------------
    ...
    def retrieveFaceControls(self,context,csvData=dict(),fID=0):
     ...
     if (doIt):
      ...
      #-----------------------------------------------------------------
      scene['neckZ'] = r["neck_Zrotation"]
      scene['neckX'] = r["neck_Xrotation"]
      scene['neckY'] = r["neck_Yrotation"]
      bpy.context.scene.neckZ = scene['neckZ']
      bpy.context.scene.neckX = scene['neckX']
      bpy.context.scene.neckY = scene['neckY']
      #-----------------------------------------------------------------
    ...
    def neckUpdate(self, context):
            ...
            FaceBVHAnimation.setSkeleton(context,"neck",neckX,neckY,neckZ)           
            target_obj   = bpy.context.scene.objects.get(bpy.context.scene.mnetTarget) 
...
def unregister():
    ...
    #--------------------------------------------------------------------------------------------------------------------------------------------
    del bpy.types.Scene.neckZ
    del bpy.types.Scene.neckX
    del bpy.types.Scene.neckY
    #--------------------------------------------------------------------------------------------------------------------------------------------
